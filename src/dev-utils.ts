...
export abstract class DevUtils {
  ...
  static async generateApiDocsAndUpdateReadme(): Promise<void> {
    ...
    await FsUtils.replaceContentInFile(README_MD_FILE, /<!-- API start -->[\s\S]*<!-- API end -->/m, () => apiDocsContentPromise);
    await FsUtils.addSurrounding(README_MD_FILE, /\*\*`example`\*\*([\s\S]*?)###/gm, '**`example`**\n```javascript\n', '```\n###');
  }
